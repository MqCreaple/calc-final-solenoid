...
class CoordTransFromStackToCirc(ThreeDScene):
    ...
    def construct(self):
        ...
        for i in np.linspace(-STACK_HEIGHT, STACK_HEIGHT, 20):
            current_vecs_20.append(Vector(direction=[0, .4, 0], color=YELLOW).move_to([0, 0, i]))
        for i in np.linspace(-STACK_HEIGHT, STACK_HEIGHT, 50):
            current_vecs_50.append(Vector(direction=[0, .4, 0], color=YELLOW).move_to([0, 0, i]))
        ...
        for i, loc in enumerate(pt_locs[1:]):
            ...
            for j, arr in enumerate(to_pt_arrs):
                new_arr = Arrow3D(start=arr.get_start(), end=loc, color=BLUE, thickness=0.02, base_radius=.05)
                arr.generate_target()
        ...
        rvec_text = MathTex(r"\vec r").move_to(to_pt_arrs[2].get_center() + UP * .5).scale(1.5)
        ...
        cvec_text = MathTex(r"\vec c").move_to(cvec.get_center() + RIGHT * 1).scale(1.5)
        cvec_with_annote = VGroup(cvec, cvec_text)
        orig_coord_objs = VGroup(axes, x_label, y_label, z_label, wire_stack_text, current_vec_vg_50, current_vec_vg_20, xy_plane, cvec_with_annote)
        ROTATW_TOT_TM = 8.0
        ROT_DIVS = 10
        UNIT_TM = ROTATW_TOT_TM / ROT_DIVS
        UNIT_ANG = (2 * PI) / ROT_DIVS
        for ang in np.linspace(0, 2 * PI, ROT_DIVS):
            orig_coord_rot_anim = Rotate(orig_coord_objs, angle=-UNIT_ANG, about_point=CIRC_CENT, axis=OUT)

            
            # change of rvec
            vecto_circum = CIRC_R * np.array([np.sin(ang-UNIT_ANG), np.cos(ang-UNIT_ANG), 0])
            circum_unit_tan = np.array(
                [np.cos(ang-UNIT_ANG), np.sin(ang-UNIT_ANG), 0])
            circum_coord = CIRC_CENT + vecto_circum
            new_rvec = Arrow(start=circum_coord, end=init_pt.get_center(), color=BLUE)
            new_rtext_pos = CIRC_CENT + vecto_circum / 2 + circum_unit_tan * .1 # move the text to the mid point of that rvec
